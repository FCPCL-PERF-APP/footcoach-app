...
export default function FicheJoueurPage() {
  ...
  const { isCoach, isAdjoint, canComment, canEdit } = useAuth()
  ...
  async function saveTest() {
    ...
    // Met à jour aussi la VMA du joueur
  ...
  async function addComment() {
    ...
    const { data: staff } = await supabase.from('staff').select('nom,prenom,role').eq('auth_id', user?.id).single()
  ...
  const rpeAvg = RPE_ITEMS.map(item => {
    ...
    return { label: item.label, value: vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : 0, color: rpeColor(vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : 0) }
  })
  ...
  const cartons = statsHistory.filter(r => r.carton_jaune).length
  ...
  return (
    <div style={{ padding: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <button onClick={() => navigate('/joueurs')} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 20, color: '#6B7280' }}>←</button>
        <Avatar initials={initials} bg={col.bg} color={col.color} size={48} />
        <div>
          <p style={{ fontSize: 16, fontWeight: 700 }}>{joueur.nom} {joueur.prenom}</p>
          <p style={{ fontSize: 12, color: '#9CA3AF' }}>{joueur.poste} {joueur.numero ? `· N°${joueur.numero}` : ''} · Groupe {joueur.groupe}</p>
        </div>
      </div>
        ...
        <>
          <Card>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <p style={{ fontSize: 13, fontWeight: 600 }}>Informations personnelles</p>
              {canEdit && (
                <Button size="sm" onClick={() => editing ? saveIdentite() : setEditing(true)} variant={editing ? 'success' : 'default'}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {[
                { label: 'Nom', key: 'nom' }, { label: 'Prénom', key: 'prenom' },
                { label: 'Date naissance', key: 'date_naissance', type: 'date' },
                { label: 'N° Licence', key: 'licence' },
                { label: 'Pied fort', key: 'pied' }, { label: 'Poste', key: 'poste' },
              ].map(f => (
          ...
          <Card>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <Input label="Contact urgence — Nom" value={form.contact_urgence_nom || ''} onChange={v => setForm(p => ({ ...p, contact_urgence_nom: v }))} disabled={!editing} />
              <Input label="Contact urgence — Tél." value={form.contact_urgence_tel || ''} onChange={v => setForm(p => ({ ...p, contact_urgence_tel: v }))} disabled={!editing} />
            </div>
          </Card>
      ...
      {activeTab === 'physio' && (
        <>
          <Card>
            ...
            {fcReserve && (
              <>
                <p style={{ fontSize: 11, fontWeight: 600, marginBottom: 8 }}>Zones FC (méthode Karvonen)</p>
                {[
          ...
          <Card>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
              <div style={{ background: '#F9FAFB', borderRadius: 10, padding: 10, textAlign: 'center' }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#185FA5' }}>{joueur.vma || '—'}</div>
                <div style={{ fontSize: 10, color: '#9CA3AF' }}>VMA actuelle (km/h)</div>
              </div>
              <div style={{ background: '#F9FAFB', borderRadius: 10, padding: 10, textAlign: 'center' }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#3B6D11' }}>
                  {tests.length >= 2 ? `+${(tests[0]?.vma - tests[tests.length - 1]?.vma).toFixed(1)}` : '—'}
                </div>
                <div style={{ fontSize: 10, color: '#9CA3AF' }}>Progression saison</div>
              </div>
            </div>
            {showAddTest && canEdit && (
              <div style={{ background: '#F9FAFB', borderRadius: 10, padding: 10, marginBottom: 10 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  ...
                  <Select label="Protocole" value={testForm.protocole} onChange={v => setTestForm(p => ({ ...p, protocole: v }))}
                    options={['Vameval','Cooper','Yo-Yo','Course navette 20m','Terrain perso']} />
                ...
                <Input label="Conditions / commentaire" value={testForm.conditions} onChange={v => setTestForm(p => ({ ...p, conditions: v }))} />
            ...
            {tests.map(t => (
              <div key={t.id} style={{ padding: '8px 0', borderBottom: '0.5px solid #F3F4F6' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div>
                    <p style={{ fontSize: 12, fontWeight: 600 }}>{t.protocole} — {t.date_test ? format(parseISO(t.date_test), 'd MMM yyyy', { locale: fr }) : '—'}</p>
                    {t.conditions && <p style={{ fontSize: 11, color: '#9CA3AF' }}>{t.conditions}</p>}
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ fontSize: 15, fontWeight: 700, color: '#185FA5' }}>{t.vma} km/h</p>
                    {t.fc_max_atteinte && <p style={{ fontSize: 10, color: '#9CA3AF' }}>FC max: {t.fc_max_atteinte} bpm</p>}
                  </div>
                </div>
              </div>
            ))}
          ...
          <Card>
                ...
                {poidsHistory.map((p, i) => {
                  ...
                  return (
                    <div key={p.id} title={`${p.poids} kg — ${p.date_mesure}`}
                      style={{ flex: 1, background: '#185FA5', borderRadius: '3px 3px 0 0', height: `${h}px`, opacity: 0.7 + (i / poidsHistory.length) * 0.3 }} />
                  )
                })}
              </div>
            )}
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
              ...
              <Button variant="primary" onClick={savePoids} style={{ marginBottom: 0, flexShrink: 0 }}>+ Ajouter</Button>
            </div>
          </Card>
      ...
      {activeTab === 'perf' && (
        <>
          ...
          <Card>
            <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Historique RPE</p>
            {rpeHistory.map(r => {
              const avg = (RPE_ITEMS.reduce((s, i) => s + (r[i.key] || 0), 0) / RPE_ITEMS.length).toFixed(1)
              return (
                <div key={r.id} style={{ marginBottom: 10, paddingBottom: 10, borderBottom: '0.5px solid #F3F4F6' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <p style={{ fontSize: 12, fontWeight: 600 }}>{r.evenements?.titre}</p>
                    <span style={{ fontSize: 12, fontWeight: 700, color: rpeColor(parseFloat(avg)) }}>{avg}/5</span>
                  </div>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {RPE_ITEMS.map(item => (
                      <div key={item.key} style={{ background: '#F9FAFB', borderRadius: 6, padding: '3px 6px', fontSize: 10 }}>
                        <span style={{ color: '#9CA3AF' }}>{item.label.split(' ')[0]} </span>
                        <span style={{ fontWeight: 600, color: rpeColor(r[item.key] || 0) }}>{r[item.key] ?? '—'}</span>
                      </div>
                    ))}
                  </div>
                  {r.commentaire && <p style={{ fontSize: 11, color: '#6B7280', marginTop: 4, fontStyle: 'italic' }}>💬 {r.commentaire}</p>}
                </div>
              )
            })}
          </Card>
              ...
              : footHistory.map(f => (
                  <div key={f.id} style={{ marginBottom: 8, paddingBottom: 8, borderBottom: '0.5px solid #F3F4F6' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 4 }}>
                      {[['Distance', `${f.distance_km}km`],['Sprints', f.sprints],['Ballons', f.ballons_touches],['V.max', `${f.vitesse_max}km/h`],['Accél.', f.accelerations]].map(([lbl, val]) => (
                        <div key={lbl} style={{ background: '#F9FAFB', borderRadius: 6, padding: '4px 6px', textAlign: 'center' }}>
      ...
      {activeTab === 'stats' && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 12 }}>
            {[['Matchs', statsHistory.length],['Buts', totalButs],['Passes déc.', totalPD],['Min. jeu', totalMin],['Note moy.', noteMoy],['Cartons 🟡', cartons]].map(([lbl, val]) => (
                  ...
                  <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '0.5px solid #F3F4F6' }}>
                    <div>
                      ...
                      <p style={{ fontSize: 11, color: '#9CA3AF' }}>
                        {s.evenements?.date_heure ? format(parseISO(s.evenements.date_heure), 'd MMM', { locale: fr }) : ''} · {s.temps_jeu}min · {s.titulaire ? 'Titu.' : 'Rempl.'} {s.carton_jaune ? '🟡' : ''}{s.carton_rouge ? '🔴' : ''}
                      </p>
                    </div>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                      ...
                      <div style={{ textAlign: 'center' }}><div style={{ fontSize: 13, fontWeight: 700 }}>{s.passes_decisives || 0}</div><div style={{ fontSize: 9, color: '#9CA3AF' }}>PD</div></div>
                    </div>
                  </div>
                ))
            }
      ...
      {activeTab === 'commentaires' && (
        <>
          {canComment && (
            <Card>
              <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Ajouter un commentaire</p>
          ...
          {commentaires.length === 0
            ? <Card><p style={{ fontSize: 13, color: '#9CA3AF', textAlign: 'center', padding: 16 }}>Aucun commentaire pour l'instant.</p></Card>
            : commentaires.map(c => (
                <Card key={c.id}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: 10, color: '#9CA3AF' }}>
                        {c.auteur_role} · {c.created_at ? format(parseISO(c.created_at), 'd MMM HH:mm', { locale: fr }) : ''}
                      </p>
                    </div>
                    <Badge type={c.auteur_role === 'coach' ? 'coach' : c.auteur_role === 'gardien' ? 'gardien' : 'adjoint'} style={{ marginLeft: 'auto' }}>
                      {c.auteur_role}
                    </Badge>
                  </div>
                  <p style={{ fontSize: 13, lineHeight: 1.5, color: '#374151' }}>{c.contenu}</p>
                </Card>
              ))
          }
        </>
